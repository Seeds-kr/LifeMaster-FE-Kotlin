...
class DetoxViewModel @Inject constructor(
    ...
    private val _timeLockItems =
        MutableStateFlow<DataResource<List<DetoxTimeLockResponse>>>(DataResource.Idle)
    val timeLockItems = _timeLockItems.asStateFlow()

    private val _permanentLockItems =
        MutableStateFlow<DataResource<List<String>>>(DataResource.Idle)
    ...
    fun fetchTimeLockItems() {
        viewModelScope.launch {
            ...
            detoxRepository.fetchTimeLockItems()
                .onSuccess {
    ...
    val remainingApps = combine(_installedApps, _timeLockItems) { installedApps, timeLockResource ->
        if (timeLockResource is DataResource.Success) {
            val timeLockedPackageNames =
                timeLockResource.data.map { it.lockedAppPackageName }.toSet()
    ...
    private fun generatePermanentLock(request: DetoxPermanentLock) {
        viewModelScope.launch {
            ...
            detoxRepository.generatePermanentLock(request = request)
                .onSuccess {
    ...
    fun fetchPermanentLockItems() {
        viewModelScope.launch {
            ...
            detoxRepository.fetchPermanentLockItems()
                .onSuccess {
    ...
    private fun updatePermanentLockItems(request: DetoxPermanentLock) {
        viewModelScope.launch {
            ...
            detoxRepository.updatePermanentLockItems(request = request)
                .onSuccess {
    ...
    fun savePermanentLock(request: DetoxPermanentLock) {
        when (_permanentLockItems.value) {
            is DataResource.Success -> {
                updatePermanentLockItems(request)
            }
            else -> {
                generatePermanentLock(request)
            }
        }
    }
    ...
    private fun generateRepeatLock(request: DetoxRepeatLock) {
        viewModelScope.launch {
            ...
            detoxRepository.generateRepeatLock(request = request)
                .onSuccess {
    ...
    fun generateTimeLock(request: DetoxTimeLockRequest) {
        viewModelScope.launch {
            _generateTimeLockResult.emit(DataResource.Loading)
            detoxRepository.generateTimeLock(request = request)
                .onSuccess {
    ...
    fun deleteTimeLockItem(id: Long) {
        viewModelScope.launch {
            ...
            detoxRepository.deleteTimeLockItem(id = id)
                .onSuccess {
