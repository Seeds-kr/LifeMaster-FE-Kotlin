...
import com.example.lifemaster.presentation.home.alarm.view.fragment.AlarmRandomMissionTypingFragmentArgs
...
class PomodoroEscapeFragment : Fragment(R.layout.fragment_pomodoro_escape) {
    ...
    private val questionList by lazy {
        listOf(binding.tvPomodoroEscapeQuestionFirst, binding.tvPomodoroEscapeQuestionSecond, binding.tvPomodoroEscapeQuestionThird)
    }
    private val answerList by lazy {
        listOf(binding.etPomodoroEscapeAnswerFirst, binding.etPomodoroEscapeAnswerSecond, binding.etPomodoroEscapeAnswerThird)
    }
    ...
    private fun initViews() = with(binding) {
        ...
        tvPomodoroEscapeTypingDate.text = "${localDateTime.year}년 ${localDateTime.monthValue}월 ${localDateTime.dayOfMonth}일"
        tvPomodoroEscapeTypingTime.text = String.format("%02d:%02d", localDateTime.hour, localDateTime.minute)
        tvPomodoroEscapeTypingAmPm.text = if(localDateTime.hour in 0..11) "am" else "pm"
    ...
    private fun initListeners() = with(binding) {
        btnPomodoroEscapeTypingNextPage.setOnClickListener {
            if (etPomodoroEscapeAnswerFirst.text.toString() == tvPomodoroEscapeQuestionFirst.text.toString()
                ...
                if(currentPage < 2) {
                    val action = PomodoroEscapeFragmentDirections.actionPomodoroEscapeFragmentSelf(currentPageNum = currentPage + 1)
            ...
            } else if (etPomodoroEscapeAnswerFirst.text.isBlank() || etPomodoroEscapeAnswerSecond.text.isBlank() || etPomodoroEscapeAnswerThird.text.isBlank()) {
    ...
    private fun initObservers() = with(binding) {
        answerList.forEachIndexed { position, answerEditText ->
            ...
            answerEditText.onFocusChangeListener = View.OnFocusChangeListener { _, hasFocus ->
                if(hasFocus) {
                    if(answerEditText.text.isEmpty()) pomodoroViewModel.increaseSentenceCount()
                } else {
                    if(answerEditText.text.isEmpty()) pomodoroViewModel.decreaseSentenceCount()
                }
            }
        }
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                launch {
                    pomodoroViewModel.writtenSentenceCount.collect { currentCount ->
                        btnPomodoroEscapeTypingNextPage.text = "${currentCount}/${TOTAL_SENTENCE_COUNT} 진행 중"
                    }
                }
            }
        }
