...
class CommunityViewModel : ViewModel() {
    ...
    private val _sortMode = MutableLiveData(SortMode.LATEST)
    ...
    private fun resortMainIfNeeded() {
        if (_sortMode.value != SortMode.LIKES) return
    ...
    private fun primeLikeStateFromSummaries(dtos: List<PostSummaryDto>) {
        dtos.forEach { dto ->
            val id = dto.id?.toString() ?: return@forEach
            dto.likeCount?.let { likeCounts[id] = it }
            if (dto.liked == true) likedPosts.add(id) else likedPosts.remove(id)
        }
    }
        ...
        RetrofitInstance.networkService
            ...
            .enqueue(object : Callback<List<PostSummaryDto>> {
                override fun onResponse(
                    ...
                    val body = res.body().orEmpty()
                    primeLikeStateFromSummaries(body)
                    _bestItems.postValue(
                        body.mapIndexed { idx, dto ->
        ...
        RetrofitInstance.networkService
            ...
            .enqueue(object : Callback<PostDetailDto> {
                override fun onResponse(
                    ...
                    val likedNow = body.liked == true
                    val countNow = body.likeCount ?: likeCounts[id] ?: 0
                    updateLikeStateEverywhere(id, likedNow, countNow)

                    body.viewCount?.let { serverViews ->
                        mutateItem(id) { it.copy(views = serverViews) }
                    }
    ...
    fun toggleCommentLike(
        ...
        val cur = _comments.value.orEmpty().toMutableList()
        val idx = cur.indexOfFirst { it.commentId == commentId }
        if (idx < 0) return

        val before = cur[idx]
        val beforeLiked = before.liked == true
        val beforeCnt = before.likeCount ?: 0
        ...
        RetrofitInstance.networkService
            ...
            .enqueue(object : Callback<ResponseBody> {
                override fun onResponse(
                    ...
                    onError("댓글 좋아요 실패 code=${res.code()} ${res.message()}")
    ...
    private fun rollbackCommentLike(
        ...
        if (i >= 0) {
            rollback[i] = rollback[i].copy(liked = liked, likeCount = count)
            _comments.postValue(rollback)
        }
    }
